...
export function* registerUser(userRegister: IUserRegister): any {
  ...
  // @ts-ignore
  const response = yield apply(userService, 'create', [
    userRegister as UserDto
  ]);
...
export function* getUserData() {
  const user: ICurrentUser = yield select((state: any) =>
    currentUserSelector(state.authReducer)
  );

  const userData: UserCheckinsDto & ApiError = yield apply(
    checkinService,
    'getForUser',
    [{ userId: user.userId! }]
  );
...
function storeUserInfo(
  ...
  const userInfo = jwt_decode<{ id: string; roles: string[] }>(
...
function* getAllUsers(): any {
  ...
  if (allUsers && !allUsers.errors) {
    yield put({
      type: userDataActions.GetAllUsersSuccess,
